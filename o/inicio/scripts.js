function adicionarParametrosAosLinks() {
    ...
    links.forEach(link => {
        try {
            const href = link.href ? new URL(link.href, window.location.origin) : null;
            if (!href) return;

            // Adicionar parâmetros aos links
...
//Preenhcer os valores
document.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);

    // Preencher os inputs com parâmetros da URL
    const email = urlParams.get('email') || carregarDoLocalStorage('email', '');
    //const name = urlParams.get('name') || carregarDoLocalStorage('name', '');

    if (email) document.getElementById('email').value = email;
    //if (name) document.getElementById('name').value = name;

    // Atualizar links com parâmetros da URL
    adicionarParametrosAosLinks();
});


// Salvar os campos no localStorage ao enviar o formulário
function salvarCampos() {
    const emailField = document.getElementById("email").value;
    //const nameField = document.getElementById("name").value;

    salvarNoLocalStorage('email', emailField);
    //salvarNoLocalStorage('name', nameField);
}
...
function abrirComParametros() {
    ...
    //if (name) urlParams.set('cust_name', name);

    const novaUrl = '../front/?' + urlParams.toString();
    window.open(novaUrl);
}
...
function carregarDoLocalStorage(chave, valorPadrao) {
    return localStorage.getItem(chave) || valorPadrao;
}

// Carregar valores iniciais
let valor = parseFloat(carregarDoLocalStorage('valor', 100.00));
...
// Atualizar valores no DOM e salvar no localStorage
function atualizarValor() {
    ...
    salvarNoLocalStorage('valor', valor.toFixed(2)); // Salvar no localStorage
}
...
function aumentarValor() {
    if (valoresDesejados.length > 0) {
        ...
        console.warn("Nenhum valor restante para incrementar.");
    }
}
...
function verificarCampos() {
    ...
    if (!emailValido.test(emailField.value)) {
    ...
    // Tudo validado, continuar com o fluxo
    showLoading();
    return true;
}

// Função showLoading corrigida
function showLoading() {
    ...
    // Verificando e alterando DOM com segurança
